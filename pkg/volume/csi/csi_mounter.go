...
func (c *csiMountMgr) SetUpAt(dir string, fsGroup *int64) error {
	...
	// TODO (vladimirvivien) make assertion calls private inside client.
...
func (c *csiMountMgr) TearDownAt(dir string) error {
	...
	if !isDriverNameValid(driverName) {
		glog.Error(log("unmounter.TearDown failure: csi driver name extracted from path is invalid: [path=%s; driverName=%s]", dir, driverName))
		return errors.New("invalid csi driver name from path")
	}
